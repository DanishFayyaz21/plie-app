...
function SelectorDanceStyle(props) {
  ...
  const { visible, danceStyleList, setDanceStyle} = props;
...
//   }, [eventTypeList])
  ...
  useEffect(() => {
    console.log("dance stylesList",danceStyleList);
  ...
  const onApply = () => {
    ...
    mDanceStyleList.map((item) => {
      if (item.isSelected) {
        danceStyleIds.push(item);
      }
    })
    console.log("l¡sta de selecionados", danceStyleIds);
    if(danceStyleIds.length === 0)
    {
      danceStyleIds.push("Dance Style");
    }
    setDanceStyle(danceStyleIds);
    console.log("dance styles", danceStyleIds);
    onClose();
    //props.onClose()
    // setVisible(false)
  }
...
export default SelectorDanceStyle;
