...
import { View, StyleSheet, TouchableOpacity, Text, Image, ScrollView,TextInput, Keyboard,KeyboardAvoidingView } from "react-native"
...
function SelectPriceModal(props) {
  ...
  const messageErrorRange = "the range of price is  €1.00 and €999.00";
  const [messageErroPrice,setMessageErroPrice] = useState("");
  const fromPrice = useRef();
  const toPrice = useRef();

  const DROPDOWN_TIMING = 400;
  const { priceInitial, setPriceInitial, priceFinal, setPriceFinal, visible,isEdit } = props;

  useEffect(() => {
    // setDateFrom(currentDate)
    setDateType(1)
    console.log("priceInitial modal 1",priceInitial);
    console.log("priceFinal modal 1",priceFinal);
  }, [])
  ...
  const validatePriceTo = (price) => {
    validatePrice(price);
    Keyboard.dismiss();
  }
  const validateRangePrice = (price, setprice,setpriceDefault) => {
    if(price == "")
    {
      setpriceDefault();
      setMessageErroPrice("");
    ...
    else
    {
      ...
      if(numberPrice > 0.00 && numberPrice <= 999.99)
      {
        setprice();
        if(messageErrorRange != "")
        {
          setMessageErroPrice("");
        }
      }
      ...
      {
        setMessageErroPrice(messageErrorRange);
      }
    }
            ...
            <View style={styles.sty3}>
              ...
              <ScrollView showsVerticalScrollIndicator={false} nestedScrollEnabled={true}>
                ...
                <View style={styles.sty25}>
                  ...
                  <Text style={styles.sty24}>Select Event Price</Text>

                  {/* </View> */}

                  
                  <ScrollView horizontal showsHorizontalScrollIndicator={false}>
                    <Text style={{marginStart:normalize(5)}}>from :</Text>
                    <TextInput
                      style={{ width:normalize(50),borderColor: 'black', borderWidth: 0.5,marginStart:normalize(10)}}
                      ref={fromPrice}
                      placeholder="price Initial" 
                      keyboardType= "numeric" 
                      maxLength={6}
                      onSubmitEditing={() => toPrice.current.focus()}
                      //setPriceInitial(price)
                      onChangeText={(price) => validateRangePrice(price, () => setPriceInitial(price),() => setPriceInitial(""))}
                      value={ isEdit? priceInitial.toString(): priceInitial} 
                    />
                    <Text style={{marginStart:normalize(10)}}>to :</Text>
                    <TextInput
                      style={{ width:normalize(50),borderColor: 'black', borderWidth: 0.5,marginStart:normalize(10)}}
                      ...
                      maxLength={6}
                      ref={toPrice}
                      onChangeText={(price) => validateRangePrice(price, () => setPriceFinal(price),() => setPriceFinal(""))}
                      onSubmitEditing={(price) => { validatePriceTo(price)}}
                      value={ isEdit ? priceFinal.toString(): priceFinal}
                    />
                    
                  </ScrollView>
                  <Text style={{color:'red',alignSelf:'center',marginTop:normalize(2)}}>{messageErroPrice}</Text>
                </View>
